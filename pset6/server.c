...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // check request-line 
            if (strchr(line, ' ') == NULL || strstr(line, "\r\n") == NULL)
            {
                error(400);
                continue;
            }
            ...
            // out of line
            char* line_aux = strdupa(line);
            char* method = strsep(&line_aux, " ");

            // request target
            char* request_target = strsep(&line_aux, " ");

            // getting absolute_path
            char* request_target_aux  = strdupa(request_target);
            char* absolute_path = strsep(&request_target_aux, "?");

            // query
            char* query = strsep(&request_target_aux, " ");
            if (query == NULL)
            {
                query = "";
            }
            ...
            // HTTP_version
            char* http_version = strsep(&line_aux, "\r\n");
            ...
            char extension[strlen(strrchr(path, '.') + 1)];
